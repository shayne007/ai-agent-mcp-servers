...
class StockAnalysisInput(BaseModel):
    stock_codes: List[str] = Field(description="List of stock codes to analyze")
    start_date: str = Field(
        default="20210101",
        description="Start date of the analysis in format YYYYMMDD")
    end_date: str = Field(
        default="20211231",
        description="End date of the analysis in format YYYYMMDD")


@tool(args_schema=StockAnalysisInput)
def analyze_stocks(stock_codes: List[str], start_date: str,
    ...
    if isinstance(stock_codes, dict):
        stock_codes = stock_codes.get('stock_codes', [])
...
if __name__ == '__main__':
    # 示例使用
    try:
        input_stock_codes = ['688070', '301024', '300149',
                             '300546']  # 可以替换为您想要分析的股票代码列表
        results = analyze_stocks.invoke(
            {"stock_codes": input_stock_codes,
